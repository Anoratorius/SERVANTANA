/**
 * Worker Subscription Service
 * Handles worker subscription fees (12 units/year after first job)
 *
 * Unit = 1 local currency (EUR, USD, GEL, CHF, etc.)
 * Workers must pay within 24 hours of first job completion or become invisible
 */

import { prisma } from "./prisma";
import { Resend } from "resend";
import { writeAuditLog } from "./audit-log";

const resend = new Resend(process.env.RESEND_API_KEY);
const FROM_EMAIL = "Servantana <noreply@servantana.com>";
const APP_URL = "https://servantana.com";

// Subscription fee: 12 units per year (1 unit = 1 local currency)
const SUBSCRIPTION_FEE = 12;
const PAYMENT_WINDOW_HOURS = 24;

// Logo for email
const EMAIL_LOGO_BASE64 = "data:image/png;base64,";

/**
 * Generate a unique 9-digit internal ID
 * Used for internal identification of workers and customers (admin-only visible)
 */
export async function generateInternalId(): Promise<string> {
  let attempts = 0;
  const maxAttempts = 10;

  while (attempts < maxAttempts) {
    // Generate random 9-digit number (100000000 to 999999999)
    const internalId = String(Math.floor(100000000 + Math.random() * 900000000));

    // Check if already exists
    const existing = await prisma.user.findFirst({
      where: { internalId },
    });

    if (!existing) {
      return internalId;
    }

    attempts++;
  }

  throw new Error("Failed to generate unique internal ID after multiple attempts");
}

/**
 * Assign internal ID to a user if they don't have one
 */
export async function assignInternalId(userId: string): Promise<string> {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: { internalId: true },
  });

  if (user?.internalId) {
    return user.internalId;
  }

  const internalId = await generateInternalId();

  await prisma.user.update({
    where: { id: userId },
    data: { internalId },
  });

  return internalId;
}

/**
 * Get subscription fee amount based on worker's currency
 * 1 unit = 1 local currency
 */
export function getSubscriptionFeeAmount(currency: string): number {
  // All currencies use same unit value (12)
  return SUBSCRIPTION_FEE;
}

/**
 * Get currency symbol for display
 */
export function getCurrencySymbol(currency: string): string {
  const symbols: Record<string, string> = {
    EUR: "\u20AC",
    USD: "$",
    GBP: "\u00A3",
    CHF: "CHF",
    GEL: "\u20BE",
    PLN: "z\u0142",
    CZK: "K\u010D",
    SEK: "kr",
    NOK: "kr",
    DKK: "kr",
  };
  return symbols[currency] || currency;
}

/**
 * Handle first job completion for a worker
 * Sets subscription status to PENDING_PAYMENT and triggers email
 */
export async function handleFirstJobCompleted(
  workerId: string,
  bookingId: string
): Promise<void> {
  const workerProfile = await prisma.workerProfile.findUnique({
    where: { userId: workerId },
    include: {
      user: { select: { email: true, firstName: true } },
    },
  });

  if (!workerProfile) {
    throw new Error("Worker profile not found");
  }

  // Only process if this is actually the first job
  if (workerProfile.workerSubscriptionStatus !== "NONE") {
    return;
  }

  const now = new Date();
  const paymentDueAt = new Date(now.getTime() + PAYMENT_WINDOW_HOURS * 60 * 60 * 1000);

  // Update worker subscription status
  await prisma.workerProfile.update({
    where: { userId: workerId },
    data: {
      workerSubscriptionStatus: "PENDING_PAYMENT",
      firstJobCompletedAt: now,
      paymentDueAt,
    },
  });

  // Send congratulation email with payment link
  await sendFirstJobCongratulationEmail(
    workerProfile.user.email,
    workerProfile.user.firstName,
    workerProfile.currency,
    paymentDueAt
  );

  // Audit log
  await writeAuditLog({
    action: "WORKER_FIRST_JOB_COMPLETED",
    actorId: workerId,
    targetId: bookingId,
    targetType: "Booking",
    details: { paymentDueAt: paymentDueAt.toISOString() },
  });
}

/**
 * Send congratulation email after first job completion
 */
async function sendFirstJobCongratulationEmail(
  email: string,
  firstName: string,
  currency: string,
  paymentDueAt: Date
): Promise<void> {
  const feeAmount = getSubscriptionFeeAmount(currency);
  const currencySymbol = getCurrencySymbol(currency);
  const paymentUrl = `${APP_URL}/en/worker/subscription/pay`;
  const dueDate = paymentDueAt.toLocaleDateString("en-US", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
    hour: "2-digit",
    minute: "2-digit",
  });

  if (!process.env.RESEND_API_KEY) {
    console.log(`[DEV] First job email for ${email}: Payment due ${dueDate}`);
    return;
  }

  await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: "Congratulations on your first job! - Servantana",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="text-align: center; margin-bottom: 30px;">
          <img src="${EMAIL_LOGO_BASE64}" alt="SERVANTANA" width="248" height="44" style="display: block; margin: 0 auto; max-width: 100%;" />
        </div>

        <h2 style="color: #333; margin-bottom: 20px;">Congratulations, ${firstName}!</h2>

        <p style="color: #555; font-size: 16px; line-height: 1.6;">
          You've successfully completed your first job on Servantana! We're thrilled to have you as part of our community of professional workers.
        </p>

        <div style="background: #f8f9fa; border-radius: 12px; padding: 20px; margin: 30px 0;">
          <h3 style="color: #333; margin-top: 0;">Annual Subscription Fee</h3>
          <p style="color: #555; font-size: 16px; margin-bottom: 10px;">
            To continue receiving job opportunities, please complete your annual subscription payment:
          </p>
          <p style="font-size: 32px; font-weight: bold; color: #2563eb; margin: 10px 0;">
            ${currencySymbol}${feeAmount}/year
          </p>
          <p style="color: #dc2626; font-size: 14px;">
            <strong>Payment due by:</strong> ${dueDate}
          </p>
        </div>

        <div style="text-align: center; margin: 30px 0;">
          <a href="${paymentUrl}"
             style="background: linear-gradient(to right, #2563eb, #16a34a); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block;">
            Pay Subscription Now
          </a>
        </div>

        <div style="background: #fef3c7; border-radius: 8px; padding: 15px; margin: 20px 0;">
          <p style="color: #92400e; font-size: 14px; margin: 0;">
            <strong>Important:</strong> If payment is not received within 24 hours, your profile will become invisible to customers and you won't receive new job requests.
          </p>
        </div>

        <p style="color: #555; font-size: 14px; line-height: 1.6;">
          Your subscription gives you:
        </p>
        <ul style="color: #555; font-size: 14px; line-height: 1.8;">
          <li>Full visibility in customer searches</li>
          <li>Unlimited job opportunities for 12 months</li>
          <li>Access to premium features</li>
          <li>Priority customer support</li>
        </ul>

        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;" />

        <p style="color: #aaa; font-size: 12px; text-align: center;">
          &copy; ${new Date().getFullYear()} Servantana. All rights reserved.
        </p>
      </div>
    `,
  });
}

/**
 * Handle subscription payment received
 */
export async function handleSubscriptionPayment(
  workerId: string,
  transactionId: string,
  amount: number,
  currency: string
): Promise<void> {
  const now = new Date();
  const subscriptionExpiresAt = new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000); // 1 year

  const workerProfile = await prisma.workerProfile.findUnique({
    where: { userId: workerId },
    include: {
      user: { select: { email: true, firstName: true } },
    },
  });

  if (!workerProfile) {
    throw new Error("Worker profile not found");
  }

  // Update subscription status
  await prisma.workerProfile.update({
    where: { userId: workerId },
    data: {
      workerSubscriptionStatus: "ACTIVE",
      subscriptionExpiresAt,
      isVisible: true,
      paymentDueAt: null,
    },
  });

  // Send receipt email
  await sendSubscriptionReceiptEmail(
    workerProfile.user.email,
    workerProfile.user.firstName,
    amount,
    currency,
    subscriptionExpiresAt,
    transactionId
  );

  // Audit log
  await writeAuditLog({
    action: "WORKER_SUBSCRIPTION_PAID",
    actorId: workerId,
    details: {
      transactionId,
      amount,
      currency,
      expiresAt: subscriptionExpiresAt.toISOString(),
    },
  });
}

/**
 * Send receipt email after subscription payment
 */
async function sendSubscriptionReceiptEmail(
  email: string,
  firstName: string,
  amount: number,
  currency: string,
  expiresAt: Date,
  transactionId: string
): Promise<void> {
  const currencySymbol = getCurrencySymbol(currency);
  const expiryDate = expiresAt.toLocaleDateString("en-US", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
  });

  if (!process.env.RESEND_API_KEY) {
    console.log(`[DEV] Receipt email for ${email}: Subscription active until ${expiryDate}`);
    return;
  }

  await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: "Payment Receipt - Your Servantana Subscription is Active!",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="text-align: center; margin-bottom: 30px;">
          <img src="${EMAIL_LOGO_BASE64}" alt="SERVANTANA" width="248" height="44" style="display: block; margin: 0 auto; max-width: 100%;" />
        </div>

        <h2 style="color: #333; margin-bottom: 20px;">Thank you for your payment, ${firstName}!</h2>

        <p style="color: #555; font-size: 16px; line-height: 1.6;">
          Your annual subscription has been activated. You now have full access to all job opportunities on Servantana.
        </p>

        <div style="background: #f0fdf4; border: 2px solid #22c55e; border-radius: 12px; padding: 20px; margin: 30px 0;">
          <h3 style="color: #15803d; margin-top: 0;">Payment Receipt</h3>

          <table style="width: 100%; border-collapse: collapse; color: #555;">
            <tr>
              <td style="padding: 8px 0; border-bottom: 1px solid #dcfce7;">Transaction ID:</td>
              <td style="padding: 8px 0; border-bottom: 1px solid #dcfce7; text-align: right; font-family: monospace;">${transactionId}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; border-bottom: 1px solid #dcfce7;">Amount Paid:</td>
              <td style="padding: 8px 0; border-bottom: 1px solid #dcfce7; text-align: right; font-weight: bold;">${currencySymbol}${amount}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; border-bottom: 1px solid #dcfce7;">Subscription Type:</td>
              <td style="padding: 8px 0; border-bottom: 1px solid #dcfce7; text-align: right;">Annual (12 months)</td>
            </tr>
            <tr>
              <td style="padding: 8px 0;">Valid Until:</td>
              <td style="padding: 8px 0; text-align: right; font-weight: bold; color: #15803d;">${expiryDate}</td>
            </tr>
          </table>
        </div>

        <div style="background: #eff6ff; border-radius: 8px; padding: 15px; margin: 20px 0;">
          <p style="color: #1e40af; font-size: 14px; margin: 0;">
            <strong>Note:</strong> Your profile will remain visible to customers until ${expiryDate}. We'll send you a reminder before your subscription expires.
          </p>
        </div>

        <p style="color: #555; font-size: 14px; line-height: 1.6;">
          Keep this email as your receipt for tax purposes.
        </p>

        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;" />

        <p style="color: #aaa; font-size: 12px; text-align: center;">
          &copy; ${new Date().getFullYear()} Servantana. All rights reserved.
        </p>
      </div>
    `,
  });
}

/**
 * Suspend worker for non-payment (called by cron job after 24h)
 */
export async function suspendWorkerForNonPayment(workerId: string): Promise<void> {
  await prisma.workerProfile.update({
    where: { userId: workerId },
    data: {
      workerSubscriptionStatus: "SUSPENDED",
      isVisible: false,
    },
  });

  // Audit log
  await writeAuditLog({
    action: "WORKER_SUSPENDED_NONPAYMENT",
    actorId: "SYSTEM",
    targetId: workerId,
    targetType: "User",
  });
}

/**
 * Check and suspend workers with expired payment windows
 * Should be called by a cron job
 */
export async function checkAndSuspendExpiredWorkers(): Promise<number> {
  const now = new Date();

  const expiredWorkers = await prisma.workerProfile.findMany({
    where: {
      workerSubscriptionStatus: "PENDING_PAYMENT",
      paymentDueAt: { lt: now },
    },
    select: { userId: true },
  });

  for (const worker of expiredWorkers) {
    await suspendWorkerForNonPayment(worker.userId);
  }

  return expiredWorkers.length;
}

/**
 * Check and expire active subscriptions
 * Should be called by a cron job
 */
export async function checkAndExpireSubscriptions(): Promise<number> {
  const now = new Date();

  const expiredSubscriptions = await prisma.workerProfile.findMany({
    where: {
      workerSubscriptionStatus: "ACTIVE",
      subscriptionExpiresAt: { lt: now },
    },
    select: { userId: true },
  });

  for (const worker of expiredSubscriptions) {
    await prisma.workerProfile.update({
      where: { userId: worker.userId },
      data: {
        workerSubscriptionStatus: "EXPIRED",
        isVisible: false,
      },
    });

    await writeAuditLog({
      action: "WORKER_SUBSCRIPTION_EXPIRED",
      actorId: "SYSTEM",
      targetId: worker.userId,
      targetType: "User",
    });
  }

  return expiredSubscriptions.length;
}
